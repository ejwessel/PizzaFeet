const inObjectPointsPosMult = new MathLib.vec3(-1.0, 1.0, -1.0);
...
Solver.prototype = {
    ...
    applySolvePnP: function(outTensor) {
        ...
        if (verifySolvePnP) {
            var maxDist = 0.0;
            for (var i = 0; i < this.visiblePointsNum; ++i) {
                var p = new MathLib.vec3(this.visibleInObjectPoints[i * 3], this.visibleInObjectPoints[i * 3 + 1], this.visibleInObjectPoints[i * 3 + 2]);
                p = global.MathLib.rotatePoint(p, this.transform);
                p = p.add(new MathLib.vec3(this.transform[4], -this.transform[3], this.transform[5]));
                p = p.mult(inObjectPointsPosMult);

                var pos1 = this.camera.camera.worldSpaceToScreenSpace(MathLib.vec3.toEngine(p));
                var pos2 = new MathLib.vec2(1.0 - this.visibleOutTensor[i * 2 + 1] / this.camera.cameraSize.y, this.visibleOutTensor[i * 2] / this.camera.cameraSize.x);
