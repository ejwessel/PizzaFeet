...
function initCamera() {
    ...
    Camera.prototype = {
        update: function() {
            this.cameraSize = new MathLib.vec2(this.camera.renderTarget.getHeight(), this.camera.renderTarget.getWidth());
            this.intrinsics = global.MathLib.makeIntrinsicsMatrix(this.cameraSize, this.camera.fov);
        },
        calcCameraSize: function() {
            return new MathLib.vec2(this.camera.renderTarget.getHeight(), this.camera.renderTarget.getWidth());
        }
    };
...
function initOpticalFlow() {
    ...
    prevTexOptFlowSize = new MathLib.vec2(script.deviceTexture.getWidth(), script.deviceTexture.getHeight());
}
...
function onUpdate() {
    ...
    var curTexOptFlowSize = new MathLib.vec2(script.deviceTexture.getWidth(), script.deviceTexture.getHeight());
    ...
    for (var i = 0; i < SIZE; ++i) {
        if (solver[TYPE.LEFT]) {
            if (solver[TYPE.LEFT].prevVisibilityMask[i] == 0) {
                solver[TYPE.LEFT].newOutTensor[i * 2] = outTensor[i * 2];
                solver[TYPE.LEFT].newOutTensor[i * 2 + 1] = outTensor[i * 2 + 1];
            } else {
                solver[TYPE.LEFT].newOutTensor[i * 2] = newOutTensor[i * 2];
                solver[TYPE.LEFT].newOutTensor[i * 2 + 1] = newOutTensor[i * 2 + 1];
            }
        }

        if (solver[TYPE.RIGHT]) {
            if (solver[TYPE.RIGHT].prevVisibilityMask[i] == 0) {
                solver[TYPE.RIGHT].newOutTensor[i * 2] = outTensor[SIZE2 + i * 2];
                solver[TYPE.RIGHT].newOutTensor[i * 2 + 1] = outTensor[SIZE2 + i * 2 + 1];
            } else {
                solver[TYPE.RIGHT].newOutTensor[i * 2] = newOutTensor[SIZE2 + i * 2];
                solver[TYPE.RIGHT].newOutTensor[i * 2 + 1] = newOutTensor[SIZE2 + i * 2 + 1];
            }
        }
    }
    ...
    if (solver[TYPE.LEFT] && solver[TYPE.LEFT].isFootVisible) {
        for (var i = 0; i < SIZE2; ++i) {
            outTensorLeft[i] = outTensor[i];
        }
    }

    if (solver[TYPE.RIGHT] && solver[TYPE.RIGHT].isFootVisible) {
        for (var i = 0; i < SIZE2; ++i) {
            outTensorRight[i] = outTensor[i + SIZE2];
        }
    }
            ...
            if (intersectionArea / b1Area > modelConfig.kOverlayThreshold ||
                intersectionArea / b2Area > modelConfig.kOverlayThreshold) {
                    if (b1Area > modelConfig.kOverlayGreaterThreshold * b2Area) {
                        ...
                        var leftProb = 1.0;
                        for (var i = 0; i < SIZE; ++i) {
                            leftProb *= maxProbabilities[i];
                        };
                        var rightProb = 1.0;
                        for (var i = SIZE; i < 2 * SIZE; ++i) {
                            rightProb *= maxProbabilities[i];
                        };
